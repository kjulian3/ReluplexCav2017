...
from . import ReluplexUtils
from . import ReluplexNetwork

class ReluplexNetworkTF(ReluplexNetwork.ReluplexNetwork):
    def __init__(self, filename, inputName=None, outputName=None, savedModel=False, savedModelTags=[]):
        """
        Constructs a ReluplexNetworkTF object from a frozen Tensorflow protobuf or SavedModel
    ...
    def maxToRelu(self, elements, v):
        while len(elements)>=2:
            ...
            while len(elements)>1:
                ...
                self.setUpperBound(aux1, 0.0)
                e = ReluplexUtils.Equation()
                e.markAuxiliaryVariable(aux1)
                e.addAddend(1, elements[0])
                e.addAddend(-1,elements[1])
                e.addAddend(-1, a)
                e.setScalar(0.0)
                self.addEquation(e)
                ...
                if len(elements)==2 and len(outVars)==0:
                    ...
                    z = self.getNewVariable()
                aux2 = self.getNewVariable()
                self.setLowerBound(aux2, 0.0)
                self.setUpperBound(aux2, 0.0)
                e = ReluplexUtils.Equation()
                e.markAuxiliaryVariable(aux2)
                e.addAddend(1, b)
                e.addAddend(1, elements[1])
                e.addAddend(-1, z)
                e.setScalar(0.0)
    ...
    def evaluateWithoutReluplex(self, inputValues):
        """
